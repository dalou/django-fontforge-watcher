            ...
            for config in configs:
                ...
                try:
                    ...
                    if not os.path.isdir(folder_output):
                        folder_output = os.path.join(settings.SITE_ROOT, folder_output)
                        if not os.path.isdir(folder_output):
...
""" % {
                    ...
                    for format in DEFAULT_FONT_FORMATS:
                        filename = "%s/%s.%s" % (folder_output, name, format)
                    ...
                    self.print_error('Error during font generation for "%s" : %s' % (config, str(e)))
            else:
